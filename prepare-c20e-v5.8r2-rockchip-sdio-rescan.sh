...
REPORT="$REPO/c20e-analysis/c20e-v5.8r2-rockchip-rescan-$STAMP"
LOG="$REPORT/full.log"
...
cleanup() {
    ...
    if [[ $rc -ne 0 && $CARD_MUTATED -eq 1 && $CARD_MOUNTED -eq 1 ]]; then
        ...
        [[ -f "$REPORT/backups/skw_sdio_lite.ko.before-v5.8r2" ]] && cp -f "$REPORT/backups/skw_sdio_lite.ko.before-v5.8r2" "$MNT/lib/modules/$KREL/updates/c20e-seekwave/skw_sdio_lite.ko"
        [[ -f "$REPORT/backups/swt6621s_wifi.ko.before-v5.8r2" ]] && cp -f "$REPORT/backups/swt6621s_wifi.ko.before-v5.8r2" "$MNT/lib/modules/$KREL/updates/c20e-seekwave/swt6621s_wifi.ko"
        [[ -f "$REPORT/backups/c20e-seekwave-v5.7.conf.before-v5.8r2" ]] && cp -f "$REPORT/backups/c20e-seekwave-v5.7.conf.before-v5.8r2" "$MNT/etc/modules-load.d/c20e-seekwave-v5.7.conf"
        [[ -f "$REPORT/backups/extlinux.conf.before-v5.8r2" ]] && cp -f "$REPORT/backups/extlinux.conf.before-v5.8r2" "$BOOT/extlinux/extlinux.conf"
        rm -f "$MNT/etc/systemd/system/c20e-v5.8r2-capture.service"
        rm -f "$MNT/usr/local/sbin/c20e-v5.8r2-capture.sh"
        rm -f "$MNT/etc/systemd/system/multi-user.target.wants/c20e-v5.8r2-capture.service"
        if [[ -s "$REPORT/backups/default-target.before-v5.8r2" ]]; then
            OLD_TARGET="$(basename "$(cat "$REPORT/backups/default-target.before-v5.8r2")")"
            systemctl --root="$MNT" set-default "$OLD_TARGET" >/dev/null 2>&1 || true
        fi
        if [[ "$(cat "$REPORT/backups/lightdm-state.before-v5.8r2" 2>/dev/null)" == "enabled" ]]; then
            systemctl --root="$MNT" enable lightdm.service >/dev/null 2>&1 || true
        else
...
echo "=== C20e V5.8r2 Rockchip SDIO rescan isolation ==="
echo "Time:       $(date -Is)"
echo "Repo:       $REPO"
echo "SD target:  $DEV"
echo "Kernel:     $K"
...
# Replace the generic no-self-scan branch structurally instead of
# matching an escape-heavy multi-line literal. This avoids the
# over-escaped \\n bug from V5.8 and tolerates whitespace changes.
lines = s.splitlines(keepends=True)
hit = [i for i, line in enumerate(lines) if 'no need skw self scan!!' in line]
assert len(hit) == 1, f'expected exactly one no-self-scan line, found {len(hit)}'
i = hit[0]
prev = i - 1
...
while nxt < len(lines) and not lines[nxt].strip():
    nxt += 1
assert prev >= 0 and lines[prev].strip() == '#else', 'expected #else before no-self-scan line'
assert nxt < len(lines) and lines[nxt].strip() == '#endif', 'expected #endif after no-self-scan line'
replacement = [
    '#else\n',
    '\t/*\n',
...
cp -a "$BOOT/extlinux/extlinux.conf" "$REPORT/backups/extlinux.conf.before-v5.8r2"
[[ -f "$MODDIR/skw_sdio_lite.ko" ]] && cp -a "$MODDIR/skw_sdio_lite.ko" "$REPORT/backups/skw_sdio_lite.ko.before-v5.8r2"
[[ -f "$MODDIR/swt6621s_wifi.ko" ]] && cp -a "$MODDIR/swt6621s_wifi.ko" "$REPORT/backups/swt6621s_wifi.ko.before-v5.8r2"
[[ -f "$MNT/etc/modules-load.d/c20e-seekwave-v5.7.conf" ]] && cp -a "$MNT/etc/modules-load.d/c20e-seekwave-v5.7.conf" "$REPORT/backups/c20e-seekwave-v5.7.conf.before-v5.8r2"
...
readlink "$MNT/etc/systemd/system/default.target" > "$REPORT/backups/default-target.before-v5.8r2" 2>/dev/null || true
if systemctl --root="$MNT" is-enabled lightdm.service >/dev/null 2>&1; then
    echo enabled > "$REPORT/backups/lightdm-state.before-v5.8r2"
else
    echo disabled > "$REPORT/backups/lightdm-state.before-v5.8r2"
fi
...
cat > "$MNT/usr/local/sbin/c20e-v5.8r2-capture.sh" <<'EOS'
#!/bin/sh
OUT=/var/log/c20e-v5.8r2-sdio-state.log
exec >>"$OUT" 2>&1
echo "===== C20e V5.8r2 capture start ====="
date -Is
uname -a
echo
echo "--- modules ---"
...
sync
EOS
chmod 0755 "$MNT/usr/local/sbin/c20e-v5.8r2-capture.sh"

cat > "$MNT/etc/systemd/system/c20e-v5.8r2-capture.service" <<'EOS'
[Unit]
Description=C20e V5.8r2 SDIO diagnostic capture
After=systemd-modules-load.service c20e-usb-debug.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/c20e-v5.8r2-capture.sh
TimeoutStartSec=30
...
systemctl --root="$MNT" enable c20e-v5.8r2-capture.service >/dev/null
...
find "$MNT/etc/systemd/system" -maxdepth 3 -type l \( -name 'c20e-usb-debug.service' -o -name 'serial-getty@ttyGS0.service' -o -name 'c20e-v5.8r2-capture.service' -o -name 'lightdm.service' \) -printf '%p -> %l\n' | tee "$REPORT/systemd-links.txt"
...
echo
echo "PASS: C20e V5.8r2 Rockchip SDIO rescan test prepared."
...
echo "  - automatic /var/log/c20e-v5.8r2-sdio-state.log capture installed"
echo
