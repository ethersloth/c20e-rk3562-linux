...
[[ -f "$D/c20e-zram-generator.conf" ]] && install -D -m0644 "$D/c20e-zram-generator.conf" /etc/systemd/zram-generator.conf
[[ -f "$D/c20e-zram-sysctl.conf" ]] && install -D -m0644 "$D/c20e-zram-sysctl.conf" /etc/sysctl.d/90-c20e-zram.conf
