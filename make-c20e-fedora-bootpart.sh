...
mcopy -i "$OUT" "$REPO/tools/c20e-fedora-live-fixup.sh" "$REPO/overlay/c20e.preset" \
    ...
    "$REPO/overlay/c20e-logind-power-key.conf" "$REPO/overlay/c20e-zram-sysctl.conf" "$REPO/overlay/c20e-zram-generator.conf" "$REPO/overlay/c20e-usb0.nmconnection" "$REPO/overlay/99-c20e-usb0-nm-managed.rules" ::/c20e/
