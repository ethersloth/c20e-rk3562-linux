...
static int panfrost_clk_init(struct panfrost_device *pfdev)
{
	...
	/*
	 * Also enable EVERY other clock the DT lists for the GPU. Panfrost only
	 * knows a core clock and an optional "bus" clock, but RK3562 wires the
	 * GPU through four: clk_mali (SCMI), clk_gpu, clk_gpu_brg (bus bridge)
	 * and aclk_gpu. The unclaimed three stay on from the bootloader through
	 * probe, then clk_disable_unused() gates them late in boot, and the next
	 * GPU register access -- the first open of the DRM device, e.g. KWin
	 * starting -- stalls the interconnect and freezes the whole SoC with no
	 * kernel message, until the watchdog resets it. Verified on the C20e:
	 * freezes on every first open; clean with clk_ignore_unused; clean
	 * with Panfrost not probed at all.
	 */
	err = devm_clk_bulk_get_all(pfdev->dev, &pfdev->all_clks);
	if (err < 0) {
		dev_err(pfdev->dev, "get all clocks failed %d\n", err);
		goto disable_bus_clock;
	}
	pfdev->num_all_clks = err;
	err = clk_bulk_prepare_enable(pfdev->num_all_clks, pfdev->all_clks);
	if (err)
		goto disable_bus_clock;
	dev_info(pfdev->dev, "enabled all %d DT clocks\n", pfdev->num_all_clks);
...
{
	clk_bulk_disable_unprepare(pfdev->num_all_clks, pfdev->all_clks);
	clk_disable_unprepare(pfdev->bus_clock);
	clk_disable_unprepare(pfdev->clock);
}
