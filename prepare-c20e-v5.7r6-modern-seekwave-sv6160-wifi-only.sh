...
REPORT="$REPO/c20e-analysis/c20e-v5.7r6-modern-seekwave-sv6160-$STAMP"
LOG="$REPORT/full.log"
...
cleanup() {
    ...
    if [[ $rc -ne 0 && $CARD_MUTATED -eq 1 && $CARD_MOUNTED -eq 1 ]]; then
        ...
        [[ -f "$REPORT/backups/Image.card.before-v5.7r6" ]] && cp -f "$REPORT/backups/Image.card.before-v5.7r6" "$BOOT/Image"
        [[ -f "$REPORT/backups/extlinux.conf.before-v5.7r6" ]] && cp -f "$REPORT/backups/extlinux.conf.before-v5.7r6" "$BOOT/extlinux/extlinux.conf"
    ...
    if [[ $rc -ne 0 && $CONFIG_SAVED -eq 1 ]]; then
        echo "Restoring local kernel .config and pre-V5.7r6 Image."
        cp -f "$REPORT/backups/kernel.config.before-v5.7r6" "$K/.config" 2>/dev/null || true
        cp -f "$REPORT/backups/Image.build-tree.before-v5.7r6" "$IMG" 2>/dev/null || true
        if [[ -f "$REPORT/backups/Module.symvers.before-v5.7r6" ]]; then
            cp -f "$REPORT/backups/Module.symvers.before-v5.7r6" "$K/Module.symvers"
        else
            rm -f "$K/Module.symvers"
        fi
        chown "$RUN_USER:$RUN_GROUP" "$K/.config" "$IMG" "$K/Module.symvers" 2>/dev/null || true
    fi
...
echo "=== C20e V5.7r6 modern Seekwave SV6160 integration ==="
echo "Time:       $(date -Is)"
echo "Repo:       $REPO"
echo "SD target:  $DEV"
echo "Kernel:     $K"
...
cp -a "$K/.config" "$REPORT/backups/kernel.config.before-v5.7r6"
cp -a "$IMG" "$REPORT/backups/Image.build-tree.before-v5.7r6"
[[ -f "$K/Module.symvers" ]] && cp -a "$K/Module.symvers" "$REPORT/backups/Module.symvers.before-v5.7r6"
[[ -f "$K/vmlinux.symvers" ]] && cp -a "$K/vmlinux.symvers" "$REPORT/backups/vmlinux.symvers.before-v5.7r6"
cp -a "$K/.config" "$WORKCFG"
chown "$RUN_USER:$RUN_GROUP" "$WORKCFG"
CONFIG_SAVED=1
sha256sum "$IMG" | tee "$REPORT/Image.build-tree.before-v5.7r6.sha256"
...
diff -u "$REPORT/backups/kernel.config.before-v5.7r6" "$WORKCFG" > "$REPORT/kernel.config.diff" || true
...
sha256sum "$IMG" | tee "$REPORT/Image.v5.7r6.sha256"

if cmp -s "$IMG" "$REPORT/backups/Image.build-tree.before-v5.7r6"; then
...
echo
echo "===== 6b/13 reconcile stale kernel symbol metadata ====="
[[ -s "$K/vmlinux.symvers" ]] || die "Current vmlinux.symvers is missing; cannot safely repair Module.symvers."

grep -E '(^|[[:space:]])(skw_start_wifi_service|skw_stop_wifi_service)([[:space:]]|$)' "$K/Module.symvers" > "$REPORT/stale-seekwave-symvers.before.txt" 2>/dev/null || true
grep -E '(^|[[:space:]])(skw_start_wifi_service|skw_stop_wifi_service)([[:space:]]|$)' "$K/vmlinux.symvers" > "$REPORT/current-vmlinux-seekwave-symvers.txt" 2>/dev/null || true

if [[ -s "$REPORT/current-vmlinux-seekwave-symvers.txt" ]]; then
    cat "$REPORT/current-vmlinux-seekwave-symvers.txt"
    die "Current vmlinux.symvers still exports old Seekwave Wi-Fi service symbols."
fi

runu python3 - "$K/Module.symvers" "$K/vmlinux.symvers" "$REPORT/Module.symvers.reconcile.txt" <<'PY'
from pathlib import Path
import sys

module_path = Path(sys.argv[1])
vmlinux_path = Path(sys.argv[2])
report_path = Path(sys.argv[3])

old_lines = module_path.read_text(errors="replace").splitlines() if module_path.exists() else []
vm_lines = [ln for ln in vmlinux_path.read_text(errors="replace").splitlines() if ln.strip()]
...
def symbol(line):
    f = fields(line)
    return f[1] if len(f) >= 2 else None

def module_name(line):
    f = fields(line)
    return f[2] if len(f) >= 3 else None

# vmlinux.symvers is authoritative for exports that exist in the current
# vmlinux. Preserve non-vmlinux module exports from Module.symvers because
# external modules may depend on symbols from separately-built kernel modules.
vm_by_symbol = {}
for ln in vm_lines:
    sym = symbol(ln)
    if sym:
...
for ln in old_lines:
    ...
    sym = symbol(ln)
    if mod == "vmlinux":
        if sym not in vm_by_symbol:
            removed_stale_vmlinux.append(ln)
        continue
    kept_module_lines.append(ln)

combined = list(vm_by_symbol.values())
seen = set(vm_by_symbol)
for ln in kept_module_lines:
    sym = symbol(ln)
...
report_path.write_text(
    f"old Module.symvers lines: {len(old_lines)}\n"
    f"current vmlinux.symvers lines: {len(vm_lines)}\n"
    f"stale vmlinux exports removed: {len(removed_stale_vmlinux)}\n"
    f"non-vmlinux module exports preserved: {len(kept_module_lines)}\n"
    + ("\nRemoved stale vmlinux entries:\n" + "\n".join(removed_stale_vmlinux) + "\n"
       if removed_stale_vmlinux else "")
)
PY

cat "$REPORT/Module.symvers.reconcile.txt"
chown "$RUN_USER:$RUN_GROUP" "$K/Module.symvers"

if grep -Eq '(^|[[:space:]])(skw_start_wifi_service|skw_stop_wifi_service)([[:space:]]|$)' "$K/Module.symvers"; then
    grep -E '(^|[[:space:]])(skw_start_wifi_service|skw_stop_wifi_service)([[:space:]]|$)' "$K/Module.symvers" | tee "$REPORT/stale-seekwave-symvers.after.txt"
    die "Stale Seekwave Wi-Fi service exports remain in Module.symvers."
fi

echo "Symbol metadata validation: stale vmlinux exports removed; old Seekwave Wi-Fi service symbols are absent." | tee "$REPORT/symvers-validation.txt"

echo
echo "===== 7/13 build modern Seekwave BSP + Wi-Fi only against this exact kernel ====="
# V5.7r6 deliberately does NOT compile Bluetooth yet. The previous attempt
...
cmp -s "$REPORT/backups/Image.build-tree.before-v5.7r6" "$BOOT/Image" || die "Card Image no longer matches the known pre-V5.7 kernel."

cp -a "$BOOT/Image" "$REPORT/backups/Image.card.before-v5.7r6"
cp -a "$BOOT/extlinux/extlinux.conf" "$REPORT/backups/extlinux.conf.before-v5.7r6"
...
echo "Bluetooth driver is intentionally NOT compiled or installed in V5.7r6." | tee "$REPORT/test-scope.txt"
...
echo
echo "PASS: C20e V5.7r6 modern Seekwave SV6160 test prepared."
...
echo "  - Module.symvers is reconciled against current vmlinux.symvers before MODPOST"
