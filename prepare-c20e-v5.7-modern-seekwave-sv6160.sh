...
REPORT="$REPO/c20e-analysis/c20e-v5.7-modern-seekwave-sv6160-$STAMP"
LOG="$REPORT/full.log"
ROOTDEV="${DEV}4"
BOOTDEV="${DEV}3"
...
cleanup() {
    ...
    if [[ -n "${SUDO_USER:-}" ]]; then
        chown -R "$SUDO_USER":"$(id -gn "$SUDO_USER")" "$REPORT" 2>/dev/null || true
        [[ -d "$SRCROOT" ]] && chown -R "$SUDO_USER":"$(id -gn "$SUDO_USER")" "$SRCROOT" 2>/dev/null || true
    fi
...
echo "=== C20e V5.7 modern Seekwave SV6160 test ==="
echo "Time:       $(date -Is)"
echo "Repo:       $REPO"
echo "SD target:  $DEV"
echo "Kernel:     $K"
...
if grep -q '^#define CONFIG_SEEKWAVE_FIRMWARE_LOAD' "$SRC/drivers/seekwaveplatform_lite/skwutil/boot_config.h"; then
    die "Pinned driver unexpectedly forces direct /data firmware loading."
fi

echo
echo "===== 4/12 remove the old in-tree Seekwave implementation from the kernel Image ====="
"$K/scripts/config" --file "$K/.config" --disable SEEKWAVE_BSP_DRIVERS
"$K/scripts/config" --file "$K/.config" --disable SKW_SDIOHAL
"$K/scripts/config" --file "$K/.config" --disable SKW_BSP_UCOM
"$K/scripts/config" --file "$K/.config" --disable SKW_BSP_BOOT
"$K/scripts/config" --file "$K/.config" --disable WLAN_VENDOR_SEEKWAVE
"$K/scripts/config" --file "$K/.config" --disable SKW_VENDOR
"$K/scripts/config" --file "$K/.config" --disable SKW_BT
...
if [[ -n "${SUDO_USER:-}" ]]; then
    chown -R "$SUDO_USER":"$(id -gn "$SUDO_USER")" "$REPORT" "$REPORT.tar.gz" "$REPORT.tar.gz.sha256" "$SRCROOT" 2>/dev/null || true
fi

echo
echo "PASS: C20e V5.7 modern Seekwave SV6160 test prepared."
echo
echo "What changed:"
