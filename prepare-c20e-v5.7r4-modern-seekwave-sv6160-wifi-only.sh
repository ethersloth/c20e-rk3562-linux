...
REPORT="$REPO/c20e-analysis/c20e-v5.7r4-modern-seekwave-sv6160-$STAMP"
LOG="$REPORT/full.log"
...
cleanup() {
    ...
    if [[ $rc -ne 0 && $CARD_MUTATED -eq 1 && $CARD_MOUNTED -eq 1 ]]; then
        ...
        [[ -f "$REPORT/backups/Image.card.before-v5.7r4" ]] && cp -f "$REPORT/backups/Image.card.before-v5.7r4" "$BOOT/Image"
        [[ -f "$REPORT/backups/extlinux.conf.before-v5.7r4" ]] && cp -f "$REPORT/backups/extlinux.conf.before-v5.7r4" "$BOOT/extlinux/extlinux.conf"
    ...
    if [[ $rc -ne 0 && $CONFIG_SAVED -eq 1 ]]; then
        echo "Restoring local kernel .config and pre-V5.7r4 Image."
        cp -f "$REPORT/backups/kernel.config.before-v5.7r4" "$K/.config" 2>/dev/null || true
        cp -f "$REPORT/backups/Image.build-tree.before-v5.7r4" "$IMG" 2>/dev/null || true
...
echo "=== C20e V5.7r4 modern Seekwave SV6160 integration ==="
echo "Time:       $(date -Is)"
echo "Repo:       $REPO"
echo "SD target:  $DEV"
echo "Kernel:     $K"
...
cp -a "$K/.config" "$REPORT/backups/kernel.config.before-v5.7r4"
cp -a "$IMG" "$REPORT/backups/Image.build-tree.before-v5.7r4"
cp -a "$K/.config" "$WORKCFG"
chown "$RUN_USER:$RUN_GROUP" "$WORKCFG"
CONFIG_SAVED=1
sha256sum "$IMG" | tee "$REPORT/Image.build-tree.before-v5.7r4.sha256"
...
diff -u "$REPORT/backups/kernel.config.before-v5.7r4" "$WORKCFG" > "$REPORT/kernel.config.diff" || true
...
sha256sum "$IMG" | tee "$REPORT/Image.v5.7r4.sha256"
...
# V5.7r4 deliberately does NOT compile Bluetooth yet. The previous attempt
...
cmp -s "$REPORT/backups/Image.build-tree.before-v5.7r4" "$BOOT/Image" || die "Card Image no longer matches the known pre-V5.7 kernel."

cp -a "$BOOT/Image" "$REPORT/backups/Image.card.before-v5.7r4"
cp -a "$BOOT/extlinux/extlinux.conf" "$REPORT/backups/extlinux.conf.before-v5.7r4"
...
echo "Bluetooth driver is intentionally NOT compiled or installed in V5.7r4." | tee "$REPORT/test-scope.txt"
...
echo
echo "PASS: C20e V5.7r4 modern Seekwave SV6160 test prepared."
